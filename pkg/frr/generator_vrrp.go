...
func GenerateVRRPConfig(cfg *VRRPConfig) (string, error) {
	...
	for _, group := range cfg.Groups {
		if group.Interface == "" {
			return "", fmt.Errorf("VRRP group %d missing interface", group.ID)
		}
		if group.ID < 1 || group.ID > 255 {
			return "", fmt.Errorf("VRRP group id must be 1-255, got %d", group.ID)
		}
		if group.VirtualAddress == "" || net.ParseIP(group.VirtualAddress) == nil {
			return "", fmt.Errorf("VRRP group %d has invalid virtual address %q", group.ID, group.VirtualAddress)
		}
		if group.Priority < 0 || group.Priority > 254 {
			return "", fmt.Errorf("VRRP group %d priority must be 1-254 when configured, got %d", group.ID, group.Priority)
		}
