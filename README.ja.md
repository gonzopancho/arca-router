...
- ✅ **Proto コンパイル & gRPC 完全配線**: 生成済み `api/v1/router.proto` スタブをデーモンと CLI に接続
- ✅ **FRR Transactional Apply**: 標準の FRR 適用経路は `transactional` management commit、復旧用に `file` も利用可能
- ✅ **v2 包括テスト**: engine、diff、plugins、gRPC server/client のユニットテスト
- ✅ **監視/オブザーバビリティ**
  - Prometheus metrics endpoint
  - Health endpoint
  - Grafana dashboard
  - 任意の read-only SNMPv2c endpoint
...
🚧 **現行リリース（v0.5.x）**: VPP 24.10+ と FRR 8.0+ が必要です
