...
An HTTP-only collector example is included in `examples/nms`. It can pass include-default, path, cardinality, payload schema, or encoding filters into telemetry catalog discovery, and can exclude selected paths, aliases, cardinalities, or payload schema IDs before requesting a bounded snapshot; exclusions resolve catalog aliases before matching metadata.
