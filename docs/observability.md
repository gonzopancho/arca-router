...
An HTTP-only collector example is included in `examples/nms`. It can pass include path, cardinality, or payload schema filters into telemetry catalog discovery, and can exclude selected cardinalities or payload schema IDs before requesting a bounded snapshot.
