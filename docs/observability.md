...
An HTTP-only collector example is included in `examples/nms`. It can discover paths from the telemetry catalog and exclude selected cardinalities or payload schema IDs before requesting a bounded snapshot.
