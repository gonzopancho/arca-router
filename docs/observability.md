...
`/api/nms/v1/telemetry/snapshot` returns a one-shot structured telemetry snapshot for HTTP-only collectors. Use repeated `path` query parameters, for example `?path=/system&path=/interfaces`; omitting `path` uses the same default telemetry paths as the gRPC stream. `timeout` defaults to `5s` and is capped at `30s`. `max_payload_bytes` defaults to `8388608` and is capped at `67108864`; the response echoes `payload_bytes`, `max_payload_bytes`, and `timeout_ms`.

An HTTP-only collector example is included in `examples/nms`.
