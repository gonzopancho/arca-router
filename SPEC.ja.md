...
内部 Unix socket gRPC API には structured streaming telemetry 用の `TelemetryService.SubscribeTelemetry` を含みます。Event は `arca.telemetry.v1` envelope を使い、`sequence`、`timestamp`、`path`、`event_type`、`encoding`、`json_payload`、`payload_bytes` を持ちます。Payload は JSON です。Subscription は path の選択、sample interval、one-shot snapshot を指定できます。Path を空にすると `/system` と `/config/running` を default として配信します。
