...
v0.8 の management-plane model には EVPN/VXLAN VNI intent を追加しています。L2 / L3 VNI の parser、serializer、validation、clone、conversion、diff、NETCONF XML/YANG は実装済みです。FRR EVPN control-plane apply と VPP VXLAN dataplane apply はまだ有効化していないため、EVPN/VXLAN 変更を含む commit は southbound integration が実装されるまで validation で拒否します。
