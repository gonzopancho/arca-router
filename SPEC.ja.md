...
`examples/nms` には status、telemetry catalog、bounded telemetry snapshot endpoint 用の標準ライブラリ HTTP collector example を含めています。この example は telemetry catalog に指定 path または alias、cardinality、payload schema ID の include filter を渡して snapshot path を導出でき、snapshot 要求前に `per-route` など指定した cardinality や `arca.telemetry.routes.v1` など指定した payload schema ID を除外することもできます。
