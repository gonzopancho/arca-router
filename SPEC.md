...
`examples/nms` includes a standard-library HTTP collector example for the status, telemetry catalog, and bounded telemetry snapshot endpoints. The example can discover paths from the telemetry catalog and exclude selected cardinalities, such as `per-route`, or selected payload schema IDs, such as `arca.telemetry.routes.v1`, before requesting a snapshot.
